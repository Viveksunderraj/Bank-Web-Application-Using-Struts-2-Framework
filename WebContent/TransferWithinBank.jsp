...
$(document).ready(function(){
	...
	$('#TransferWithinBank').addClass('active');
...
if(!loggedIn) {
	...
	ArrayList<Beneficiary> beneficiaryList = BeneficiaryDAO.getBeneficiary(Integer.parseInt(customerid), 1);
	request.setAttribute("beneficiaryList",beneficiaryList);
}

%>  
	<form action="transferWithinBank" method="post">
	<div>
	<br><br><h3>TRANSFER WITHIN BANK</h3><br>
		...
		<div class="col-6">
			<select name="ToAccountnumber" id="toAccountList">
				<c:forEach items="${beneficiaryList}" var="beneficiary"> 
					<option value="${beneficiary.getBeneficiaryAccountNumber()}">${beneficiary.getBeneficiaryAccountNumber()}</option><br><br>
				</c:forEach>
			</select>
		</div>
	</div>
